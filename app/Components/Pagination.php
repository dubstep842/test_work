...
class Pagination
{
    ...
    public function get()
    {
        ...
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            ...
            if ($page == $this->current_page) {
                $links .= "<li aria-current=\"page\" class=\"page-item active\"><span class=\"page-link\">$page</span></li>";
            } else {
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        $currentURI = rtrim($_SERVER['REQUEST_URI'], "/{$_SERVER['QUERY_STRING']}");
        if ($_SERVER['QUERY_STRING'] == '') {
            $_SERVER['QUERY_STRING'] .= '?page=0';
        } elseif (stripos($_SERVER['QUERY_STRING'], 'page=') === false) {
            $_SERVER['QUERY_STRING'] .= '&page=0';
        }
        $currentURI .= preg_replace('~page=[0-9]+~', "page=$page", $_SERVER['QUERY_STRING']);

        # Формируем HTML код ссылки и возвращаем
        return "<li class=\"page-item\"><a href=\"{$currentURI}\"class=\"page-link\">{$text}</a></li>";
    }
